...
sudo docker run -d --restart unless-stopped --network host -e DRONE_WEIGHTS="$WEIGHTS" -e DRONE_RECORD_DIR=/app/recordings \
  -e DRONE_CONF="${DRONE_CONF:-0.02}" -e DRONE_REPORT_MIN_CONF="${DRONE_REPORT_MIN_CONF:-0.01}" \
  ...
  -e DRONE_DIVE_MIN_TRACK_CONF="${DRONE_DIVE_MIN_TRACK_CONF:-0.08}"   -e DRONE_MOTION_ONLINE_MIN="${DRONE_MOTION_ONLINE_MIN:-8}" -e DRONE_DISAGREE_PX="${DRONE_DISAGREE_PX:-10}" -e DRONE_SWEEP_FOLLOW="${DRONE_SWEEP_FOLLOW:-1}" \
